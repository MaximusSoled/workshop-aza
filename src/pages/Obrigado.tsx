...
const Obrigado = () => {
  ...
  const handleWhatsAppRedirect = (e: React.MouseEvent) => {
    e.preventDefault();
    const whatsappUrl = "https://chat.whatsapp.com/EhmeNZoLHOkGNxFS8N0bqU";
    ...
    // Client-side pixel tracking (fallback for standard Meta Pixel)
    try {
    ...
    // Redirect fallback timer (in case API takes longer than 900ms)
    const timeout = setTimeout(() => {
      window.location.href = whatsappUrl;
    }, 900);

    // Server-side Conversions API (CAPI) and GA4 event logging
    fetch('/tracker', {
      method: 'POST',
                ...
                <div className="relative" style={{ background: "hsl(206 15% 13%)" }}>
                  ...
                  >
                    ...
                    <ShinyButton
                      ...
                      className="w-full max-w-[260px] !py-3 !px-4 flex items-center justify-center gap-2.5 font-bold"
                    >
                      {/* WhatsApp icon */}
