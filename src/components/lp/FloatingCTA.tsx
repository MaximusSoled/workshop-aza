    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          {/* CTA content */}
            <div className="relative pointer-events-auto px-5 pb-3 pt-6 max-w-md mx-auto space-y-1.5">
              <button
                onClick={() => document.getElementById("oferta")?.scrollIntoView({ behavior: "smooth" })}
                className="hero-cta-btn group w-full !py-2.5 !px-5"
              >
                ...
                <span className="hero-cta-price">
                  <span className="text-[9px] uppercase tracking-wider opacity-80 leading-none">Por apenas:</span>
                  <span className="text-xl font-black leading-none">R$59,90</span>
                </span>
              </button>
              <div className="flex items-center gap-2">
                <div className="h-1 rounded-full overflow-hidden bg-border/50 flex-1">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: "20%" }}
                ...
                <span className="text-[10px] font-bold text-electric min-w-[28px] text-right">20%</span>
              </div>
            </div>
        </motion.div>
      )}
